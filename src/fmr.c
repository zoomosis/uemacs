...

{
	...
	rg.x.cx = right - left + 1;	/* # of chars to display */
...
{
	/* separate and write this line out */
	if (right == 0) {
		charwrite(row, outstr, forg, 0, 79);
		gds_bline(row, bacg, 0, 79);
	} else {
		/* left of highleghted region */
		if (left > 0) {
			charwrite(row, outstr, forg, 0, left - 1);
			gds_bline(row, bacg, 0, left - 1);
		}

		/* highlighted region */
		charwrite(row, outstr, bacg, left, right);
		gds_bline(row, forg, left, right);

		/* right of highlighted region */
		if (right < term.t_ncol) {
			charwrite(row, outstr, forg, right + 1, term.t_ncol);
			gds_bline(row, bacg, right + 1, term.t_ncol);
		}
	}
...
gds_bline(line, color, left, right)

int line;	/* screen line to reset color on */
int color;	/* color to draw background line in */
int left, right; /* left and write boundry conditions on line */
